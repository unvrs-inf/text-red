...
export default function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
	const { settings, saveSettings, clearSettings, getDefaultSettings } =
		useSettings()
	...
	const handleTest = async () => {
		...
		try {
			const credentials = btoa(
				`${form.gigachatClientId}:${form.gigachatClientSecret}`,
			)
			...
			<div className='flex flex-col gap-4'>
				...
				<div>
					...
					<div className='relative'>
						<input
							...
							onChange={e =>
								handleChange('gigachatClientSecret', e.target.value)
							}
				...
				<div>
					...
					<select
						...
						className={inputClass}
					>
						<option value='GIGACHAT_API_PERS'>
							GIGACHAT_API_PERS (физическое лицо)
						</option>
						<option value='GIGACHAT_API_B2B'>
							GIGACHAT_API_B2B (юр. лицо)
						</option>
						<option value='GIGACHAT_API_CORP'>
				...
				<div>
					...
					>
						...
						<option value='GigaChat-2-Max'>
							GigaChat-2-Max (максимальная)
						</option>
					</select>
				</div>
				...
				<div className='bg-gray-50 dark:bg-gray-800 rounded-lg p-3 text-sm text-gray-600 dark:text-gray-400'>
					<p>
						Использовано токенов за всё время:{' '}
						<strong>{stats.totalUsed.toLocaleString('ru-RU')}</strong>
					</p>
					<p className='text-xs mt-0.5'>
						Сэкономлено (кэш): {stats.totalCached.toLocaleString('ru-RU')}
					</p>
				</div>
				...
				<p className='text-xs text-gray-400 dark:text-gray-500'>
					Ключ хранится только на вашем устройстве и передаётся только серверам
					GigaChat (Сбер).
				</p>
			</div>
		</Modal>
